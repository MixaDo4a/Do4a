...
begin
  ...
  if not (
    ...
    or app_private.current_user_has_role('manager')
  ...
  set
    status = 'closed',
    closed_by_employee_id = v_employee_id,
    closed_at = now(),
  ...
  select
    ...
    v_check_depth,
    sp.sales_percent,
    public.calculate_shift_sales_pay(v_shift.store_id, v_gross_revenue, sp.participant_role)
  ...
  perform public.log_audit_event(
    ...
    to_jsonb(v_shift),
    public.calculate_shift_snapshot_data(p_shift_id),
    'Shift closed by assigned staff, manager, or admin'
  );
